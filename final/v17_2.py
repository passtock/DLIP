...
# ==========================================
# 1. 설정 및 하이퍼파라미터 (v17 격리 저장)
# ==========================================
RES_DIR = '/data/EunJi/h22000561_psa/v17_2' 
...
print(f'>>> Running Version 17 (Multi-Head Attention Fusion) on: {DEVICE} <<<')
...
def build_dataframe(data_root):
    ...
    print(f"Total RAW cards loaded for v17: {len(df)}")
    return df
...
# ==========================================
# 3. 모델 아키텍처 (Multi-Head Attention 구현)
# ==========================================
...
if __name__ == '__main__':
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label']), 1):
        print(f'\n--- Training Fold {fold}/{CFG["num_folds"]} (v17) ---')
    ...
    # 5. 최종 앙상블 리포트 저장
    # ==========================================
    print("\n>>> Running Full Dataset Ensemble Evaluation (v17)... <<<")
    ...
    with open(f'{RES_DIR}/academic_report.txt', 'w', encoding='utf-8') as f_out:
        f_out.write("=== Final Binary Ensemble Report v17 (MHA Fusion) ===\n")
    ...
    plt.subplot(1, 3, 1)
    plt.imshow(cm, interpolation='nearest', cmap=plt.cm.Blues); plt.title('Confusion Matrix'); plt.colorbar()
    thresh = cm.max() / 2.
    for i, j in np.ndindex(cm.shape):
        plt.text(j, i, format(cm[i, j], 'd'), horizontalalignment="center", color="white" if cm[i, j] > thresh else "black")
        
    fpr, tpr, _ = roc_curve(all_labels, target_probs)
    plt.subplot(1, 3, 2); plt.plot(fpr, tpr, color='darkorange', lw=2, label=f'AUC = {auc(fpr, tpr):.4f}'); plt.plot([0, 1], [0, 1], linestyle='--'); plt.legend()
