...
# 1. 환경 설정 (v3 Fixed)
# ==========================================
VERSION = "v_ultimate_pro_3_fixed"
...
print("=======================================================")
print(f"🚀 Running {VERSION} on {DEVICE}")
print("=======================================================")

# --- 조기 종료 봇 (최적 에포크 기록 기능 추가) ---
class EarlyStopping:
...
def make_tf(mode='train'):
    ...
    if mode == 'train':
        return T.Compose([
            ...
            T.ToTensor(), norm,
            T.RandomErasing(p=0.3, scale=(0.02, 0.1), value='random')
        ])
...
if __name__ == '__main__':
    ...
    # [수정] Focal Loss 제거 -> 라벨 스무딩이 적용된 기본 손실함수로 교체 (극단적 예측 방지)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['label']), 1):
        ...
        for epoch in range(1, CFG['epochs'] + 1):
            ...
            early_stopping(vl_loss, model, epoch)
            if early_stopping.early_stop:
                print(f"🛑 [조기 종료 발동] Fold {fold} 훈련 중단.")
                break
        
        # [신규 기능] 각 폴드의 학습 요약 텍스트 저장
        fold_summary = f"Fold {fold} | Best Epoch: {early_stopping.best_epoch} | Best Val Loss: {-early_stopping.best_score:.4f}"
        training_summary.append(fold_summary)
        
        # 훈련 상세 로그(Epoch별) TXT 저장
        with open(f'{RES_DIR}/Fold_{fold}_Training_Log.txt', 'w', encoding='utf-8') as f_log:
            f_log.write(f"=== Fold {fold} Training History ===\n")
            for ep in range(len(history['train_loss'])):
                f_log.write(f"Epoch {ep+1:02d}: Train Loss: {history['train_loss'][ep]:.4f}, Val Loss: {history['val_loss'][ep]:.4f}, Val Acc: {history['val_acc'][ep]:.4f}\n")
            f_log.write(f"\n최고 성능(Best Model)은 Epoch {early_stopping.best_epoch} 에서 저장되었습니다.\n")
    ...
    # 모든 Fold 종료 후 통합 훈련 요약 보고서 저장
    with open(f'{RES_DIR}/TRAINING_SUMMARY_REPORT.txt', 'w', encoding='utf-8') as f_sum:
        f_sum.write(f"=== {VERSION} 5-Fold Training Summary ===\n\n")
        f_sum.write("\n".join(training_summary))
        f_sum.write("\n\n(상세 에포크별 로그는 각 Fold_X_Training_Log.txt 파일을 참조하세요.)")
    ...
    with open(f'{RES_DIR}/FINAL_TEST_REPORT.txt', 'w', encoding='utf-8') as f: f.write(report_txt)
    print("\n🎯 [ 최종 성적표 ]\n", report_txt)
