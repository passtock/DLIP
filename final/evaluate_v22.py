...
# ==========================================
# 1. 터미널 인자 받기 (버전명, 백본 종류)
# ==========================================
VERSION = sys.argv[1]
BACKBONE = sys.argv[2] if len(sys.argv) > 2 else 'b2'
...
def evaluate_ensemble():
    print(f"\n🚀 [{VERSION}] 5-Fold 앙상블 테스트 평가 시작 (Backbone: {BACKBONE.upper()})...")
    ...
    if not models:
        print(f"❌ {RES_DIR} 폴더에 저장된 가중치가 없습니다!")
        return

    all_preds, all_labels, all_probs_10 = [], [], []

    print("📊 채점 중... (다수결 앙상블 진행 중)")
    with torch.no_grad():
    ...
    plt.title(f'{VERSION} Ensemble Confusion Matrix')
    ...
    plt.title(f'{VERSION} ROC Curve (PSA 10 vs Rest)')
