...
# 허깅페이스 캐시 락 무효화 및 오프라인 강제 (리눅스 서버 필수)
...
# [💥중요 변경] 결과물 저장 경로를 56GB 여유 공간이 있는 EunJi 폴더 내부로 강제 격리
RES_DIR = '/data/EunJi/h22000561_psa/final_project_v11_results'
os.makedirs(RES_DIR, exist_ok=True)

SEED = 42
random.seed(SEED); np.random.seed(SEED); torch.manual_seed(SEED)
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f'>>> Running Ultimate Master Version v11 on Safe Partition: {DEVICE} <<<')
...
plt.xlabel('Predicted'); plt.ylabel('Actual'); plt.title('Confusion Matrix (Binary)')
plt.tight_layout()
