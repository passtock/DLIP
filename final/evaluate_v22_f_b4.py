...
# ==========================================
# 1. 환경 설정 (v22_f_b4 폴더 타겟팅)
# ==========================================
RES_DIR = '/data/EunJi/h22000561_psa/v22_f_b4'
...
class BranchEncoder(nn.Module):
    def __init__(self, out_dim: int = 256):
        super().__init__()
        # 평가 스크립트이므로 반드시 B4로 모델을 생성해야 가중치 크기가 맞습니다.
...
def evaluate_ensemble():
    print(f"\n🚀 [v22_f_b4] Focal + B4 앙상블 테스트 평가 시작...")
    ...
    if not models:
        print(f"❌ {RES_DIR} 폴더에 저장된 가중치가 없습니다. 학습이 완료되었는지 확인해주세요!")
        return

    all_preds, all_labels, all_probs_10 = [], [], []

    print("📊 5개의 B4 모델이 채점을 시작합니다...")
    with torch.no_grad():
    ...
    plt.title('v22_f_b4 Ensemble Confusion Matrix')
    ...
    plt.title('v22_f_b4 ROC Curve (PSA 10 vs Rest)')
