...
print(f'>>> Running HIGH-RES Raw-Data Master on: {DEVICE} <<<')

CFG = {
    'img_size'    : 300,  # 메인 중앙 및 엣지 크롭 기준 너비 (유지)
    'corner_size' : 128,  # [UP] 기존 96 -> 128 (디테일 확보)
    'edge_size'   : 48,   # [UP] 기존 32 -> 48
    'batch_size'  : 12,   # [DOWN] 해상도 증가로 인한 CUDA 메모리 부족 방지
    'epochs'      : 25,        
    'num_folds'   : 5,         
    'lr'          : 2e-4,      
    'weight_decay': 1e-2,      
...
# 1차 크롭 비율 및 [UP] 동기화 타깃 해상도 대폭 상향
TRAIN_CROP_RATIO = {'rx': 0.0974, 'ry': 0.2613, 'rw': 0.9026, 'rh': 0.9280}
TARGET_SIZE = (400, 600)  # 기존 182x268 대비 약 5배 면적 증가
...
class PSADataset(Dataset):
    def __init__(self, target_df, mode='train'):
        ...
        self.tf_full, self.tf_corner, self.tf_edge, self.tf_surface = make_tf(CFG['img_size'], mode), make_tf(CFG['corner_size'], mode), make_tf(CFG['edge_size'], mode), make_tf(CFG['img_size']//2, mode)
    ...
    def __getitem__(self, idx):
        ...
        def load_raw_and_crop(path):
            with Image.open(path).convert('RGB') as img:
                w, h = img.size
                cropped = img.crop((int(TRAIN_CROP_RATIO['rx']*w), int(TRAIN_CROP_RATIO['ry']*h), int(TRAIN_CROP_RATIO['rw']*w), int(TRAIN_CROP_RATIO['rh']*h)))
...
class PSAMultiBranchModel(nn.Module):
    def __init__(self, embed_dim=128):
        super().__init__()
        self.centering, self.corner, self.edge, self.surface = RegionEncoder(6, embed_dim), RegionEncoder(24, embed_dim), RegionEncoder(24, embed_dim), RegionEncoder(6, embed_dim)
        self.attention_fusion = AttentionFusion(dim=embed_dim)
        self.classifier = nn.Sequential(nn.Dropout(CFG['dropout']), nn.Linear(embed_dim * 4, 128), nn.GELU(), nn.Dropout(CFG['dropout']/2), nn.Linear(128, 2))
    def forward(self, f, c, e, s):
        fused = self.attention_fusion(torch.stack([self.centering(f), self.corner(c), self.edge(e), self.surface(s)], dim=1))
...
if __name__ == '__main__':
    ...
    print("\n" + "="*50 + "\n=== [RAW 고해상도 완료] 최종 앙상블 리포트 ===\n" + "="*50)
    print(classification_report(y_true, y_pred, target_names=CLASS_NAMES, digits=4))
    
    # [추가] 과하게 깐깐해진 모델을 달래줄 0.35 임계값 리포트 출력
    optimal_th = 0.35
    y_pred_opt = (target_probs >= optimal_th).astype(int)
    print(f"\n=== [Tuning] Report (Threshold {optimal_th}) ===")
    print(classification_report(y_true, y_pred_opt, target_names=CLASS_NAMES, digits=4))
    ...
    with open(f'{RES_DIR}/academic_report.txt', 'w', encoding='utf-8') as f_out:
        ...
        f_out.write(classification_report(y_true, y_pred, target_names=CLASS_NAMES, digits=4))
        f_out.write(f"\n\n=== [Tuning] Report (Threshold {optimal_th}) ===\n")
        f_out.write(classification_report(y_true, y_pred_opt, target_names=CLASS_NAMES, digits=4))
    ...
    fpr, tpr, _ = roc_curve(y_true, target_probs)
    plt.subplot(1, 3, 2)
    ...
    precision, recall, _ = precision_recall_curve(y_true, target_probs)
    plt.subplot(1, 3, 3)
