...
CFG = {
    ...
    'edge_size'   : 32,
    'batch_size'  : 24,
    'epochs'      : 50,      # UPDATED: Increased to 50 for stable convergence
    'num_folds'   : 5,
    'lr'          : 3e-4,
    'weight_decay': 1e-4,
    'dropout'     : 0.4,
...
def make_tf(size, mode='train'):
    ...
    if mode == 'train':
        return T.Compose([
            T.Resize((size, size)),
            # UPDATED: Removed 'translate' to preserve centering label accuracy
...
class RegionEncoder(nn.Module):
    def __init__(self, in_channels, out_dim=128, backbone='efficientnet_b0'):
        ...
        base.classifier = nn.Sequential(nn.Linear(feat_dim, out_dim), nn.ReLU())
        self.encoder = base
...
def train_one_epoch(model, loader, optimizer, criterion):
    ...
    for full, corners, edges, surface, labels in loader:
        full, corners, edges, surface, labels = (
            full.to(DEVICE), corners.to(DEVICE), edges.to(DEVICE), surface.to(DEVICE), labels.to(DEVICE)
        )
        optimizer.zero_grad()
...
def tta_predict(model, full, corners, edges, surface, n=8):
    ...
    for i in range(n):
        logit = model(augment(full,i), augment(corners,i), augment(edges,i), augment(surface,i))
...
val_loader_eval = DataLoader(val_ds_eval, batch_size=CFG['batch_size'], shuffle=False, num_workers=CFG['num_workers'])
...
with torch.no_grad():
    for full, corners, edges, surface, labels in val_loader_eval:
        full, corners, edges, surface = full.to(DEVICE), corners.to(DEVICE), edges.to(DEVICE), surface.to(DEVICE)
        probs_list = [tta_predict(m, full, corners, edges, surface, n=CFG['tta_n']) for m in all_models]
