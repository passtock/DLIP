...
# ==========================================
# 1. 설정 및 하이퍼파라미터 (v19 격리 저장)
# ==========================================
RES_DIR = '/data/EunJi/h22000561_psa/v19' 
...
print(f'>>> Running SOTA Version 19 (Region Transformer Network) on: {DEVICE} <<<')
...
def build_dataframe(data_root):
    ...
    print(f"Total RAW cards loaded for v19: {len(df)}")
    return df
...
# ==========================================
# 3. 모델 아키텍처 (Region Transformer 구현)
# ==========================================
...
class RegionTransformerFusion(nn.Module):
    def __init__(self, dim=128, num_heads=4):
        super().__init__()
        # 4개 영역 토큰에 위치 정보를 더해줄 학습 가능한 임베딩 파라미터
        self.pos_embed = nn.Parameter(torch.zeros(1, 4, dim))
        self.transformer = nn.TransformerEncoderLayer(
            d_model=dim, nhead=num_heads, dim_feedforward=512, 
    ...
    def forward(self, x):
        ...
        return x.reshape(x.size(0), -1) # 특징들을 1차원으로 직렬화 [B, 4 * C]
...
if __name__ == '__main__':
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label']), 1):
        print(f'\n--- Training Fold {fold}/{CFG["num_folds"]} (v19) ---')
    ...
    # ==========================================
    # 5. 최종 앙상블 리포트 및 시각화 대시보드 저장
    # ==========================================
    print("\n>>> Running Full Dataset Ensemble Evaluation (v19)... <<<")
    ...
    with open(f'{RES_DIR}/academic_report.txt', 'w', encoding='utf-8') as f_out:
        f_out.write("=== Final Binary Ensemble Report v19 (Region Transformer) ===\n")
