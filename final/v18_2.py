...
# ==========================================
# 1. 설정 및 하이퍼파라미터 (v18 격리 저장)
# ==========================================
RES_DIR = '/data/EunJi/h22000561_psa/v18_2' 
DATA_ROOT = '/data3/home/h22000561/psa_grading/data/train_yolo'
...
print(f'>>> Running Version 18 (Gated Fusion + Focal Loss) on: {DEVICE} <<<')
...
CFG = {
    ...
    'dropout'     : 0.3,       
    'num_workers' : 4,         
    'focal_gamma' : 2.0  # 애매한 9등급/10등급 구분을 위해 패널티를 강화하는 가중치
}
...
def build_dataframe(data_root):
    ...
    print(f"Total RAW cards loaded for v18: {len(df)}")
    return df
...
# ==========================================
# 3. 모델 및 손실 함수 (Gated Fusion & Focal Loss)
# ==========================================
...
class GatedFeatureFusion(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        flat = x.reshape(B, -1)
        gate_weights = self.gate(flat).unsqueeze(-1) # [B, N, 1]
...
if __name__ == '__main__':
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label']), 1):
        print(f'\n--- Training Fold {fold}/{CFG["num_folds"]} (v18) ---')
        ...
        scheduler = OneCycleLR(optimizer, max_lr=2e-4, steps_per_epoch=len(train_loader), epochs=25)
        ...
        plt.subplot(1, 2, 1); plt.plot(epochs_range, fold_history['train_loss'], label='Train Loss'); plt.plot(epochs_range, fold_history['val_loss'], label='Val Loss'); plt.title('Loss'); plt.legend()
        plt.subplot(1, 2, 2); plt.plot(epochs_range, fold_history['train_acc'], label='Train Acc'); plt.plot(epochs_range, fold_history['val_acc'], label='Val Acc'); plt.title('Acc'); plt.legend()
    ...
    # 5. 최종 앙상블 리포트 저장
    # ==========================================
    print("\n>>> Running Full Dataset Ensemble Evaluation (v18)... <<<")
    final_full_loader = DataLoader(PSADataset(df, 'val'), batch_size=16, shuffle=False, num_workers=4)
    models = []
    ...
    with open(f'{RES_DIR}/academic_report.txt', 'w', encoding='utf-8') as f_out:
        f_out.write("=== Final Binary Ensemble Report v18 (Gated + Focal) ===\n")
