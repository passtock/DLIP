...
def predict_with_tta(model, f, c, e, s):
    probs = []
    # Native Binary 출력이므로 torch.sigmoid 사용
...
def run_endgame_evaluation():
    ...
    print(f"\n✨ [탐색 완료] 황금 임계값: {best_thresh:.2f} (최고 정확도: {best_acc*100:.2f}%)")
    ...
    print(f"\n=== 🏅 ENDGAME Final Report 🏅 ===\n{rep}")
    
    # 리포트 및 그래프 저장
    with open(f"{LOAD_DIR}/final_report.txt", "w") as f: f.write(rep)
    ...
    plt.title(f'ENDGAME Matrix (Thresh: {best_thresh:.2f})'); plt.xlabel('Predicted'); plt.ylabel('Actual')
    ...
    plt.title('ENDGAME ROC Curve')
    plt.savefig(f"{LOAD_DIR}/final_graphs.png", dpi=300)
    
    print(f"✅ 최종 성적표 및 그래프 저장 완료: {LOAD_DIR}")

if __name__ == "__main__":
    run_endgame_evaluation()
